...
class ResPartner(models.Model):
    _name = 'res.partner'
    _inherit = ["res.partner", 'product.properties.print.mixin']

    print_properties = fields.One2many('product.properties.print',
                                       'partner_id',
                                       'Print properties',
                                       copy=False)
    # count_datasheets = fields.Integer('Count Datasheets',
    #                                   compute='_compute_has_datasheets')
    # print_properties = fields.One2many('product.properties.print',
    #                                    'partner_id',
    #                                    'Print properties')
    ...
    def set_all_print_properties(self):
        ...
        for record in self:
            ...
            if (print_ids or print_static_ids) and not record.print_properties:
                if print_ids:
                    print_properties += [
                        (0, False, {'name': x.id, 'partner_id': self.id, 'print': True, 'sequence': x.sequence}) for x
                        in print_ids if x.name]
                if print_static_ids:
                    print_properties += [
                        (0, False, {'static_field': x, 'partner_id': self.id, 'print': True, 'sequence': 9999}) for x in
                        print_static_ids]
    ...
    #                 </p>'''),
    #         'limit': 80,
    #         'context': "{'default_res_model': '%s','default_res_id': %d, 'partner_id': %d}" % (
    #             'res.partner', self.id, self.id)
    #     }

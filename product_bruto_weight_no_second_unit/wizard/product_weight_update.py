...
class ProductWeightUpdate(models.TransientModel):
    ...
    def calculate_product_bom_weight(self, bom, product=False):
        ...
        for bom_line, info in lines_info:
            ...
            bruto_weight += component.bruto_weight * component_qty
        if product:
            _logger.info("%s : %0.2f-%0.2f",
                         product.name,
                         weight, bruto_weight)
            product.write({'weight': weight, 'bruto_weight': bruto_weight})
        else:
            _logger.info("%s : %0.2f-%0.2f",
                         product_tmpl.name,
                         weight, bruto_weight)
            product_tmpl.write({'weight': weight, 'bruto_weight': bruto_weight})

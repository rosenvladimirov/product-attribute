...
class ProductProduct(models.Model):
    ...
    type_internal_ref = fields.Selection([('sequence', _('Use Sequence for default code')),
                                          ('build', _('Use build logic by Attributes'))], string="Usage mod for default code", default="sequence")

    @api.model
    def create(self, vals):
        if vals['type_internal_ref'] == 'sequence' and ('default_code' not in vals or vals['default_code'] == '/'):
    ...
    def write(self, vals):
        ...
        re-assignment."""
        for product in self:
            if vals.get('type_internal_ref', '') == 'sequence' and vals.get('default_code', '') == '/':
